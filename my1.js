...
if (problemForm) {
    problemForm.addEventListener("submit", function (event) {
        ...
        // Citizen Choice Guard: If duplicate score >= 40 and citizen hasn't explicitly chosen "Report Anyway"
        if (window.currentDuplicateResult && window.currentDuplicateResult.duplicate_score >= 40 && !window.overrideDuplicateReport) {
            const dupCard = document.getElementById("aiDuplicateCard");
            if (dupCard) {
                dupCard.scrollIntoView({ behavior: "smooth", block: "center" });
                dupCard.style.outline = "2px solid #f59e0b";
                setTimeout(() => { dupCard.style.outline = "none"; }, 2000);
            }
            showToast("A similar problem was found in your area. Please choose: Support This Report, Report Anyway, or View Details.");
            return;
        }
        ...
        // Attach Duplicate Detection & Issue Clustering Metadata
        const dupRes = window.currentDuplicateResult;
        const isDupScore = dupRes && dupRes.duplicate_score >= 40;
        analysis.is_duplicate = !!isDupScore;
        analysis.duplicate_score = dupRes ? dupRes.duplicate_score : 0;
        analysis.duplicate_of_id = dupRes ? (dupRes.id || dupRes.report_id) : null;
        analysis.matched_report_id = analysis.duplicate_of_id;
        analysis.issue_cluster_id = dupRes ? dupRes.issue_cluster_id : null;
        analysis.reported_anyway = !!window.overrideDuplicateReport;
        analysis.duplicate_action = window.overrideDuplicateReport ? "Reported Anyway" : (isDupScore ? "Under Review" : "None");
        analysis.user_id = typeof getOrSetCitizenUserId === "function" ? getOrSetCitizenUserId() : "Citizen";
        analysis.locality = area;
        analysis.support_count = 1;

        // Reset override flag for future submissions
        ...
        if (MatchingAPI.isBackendLive) {
            fetch(`${MatchingAPI.backendBaseUrl}/api/problems`, {
                ...
                body: JSON.stringify({
                    ...
                    reported_anyway: analysis.reported_anyway,
                    matched_report_id: analysis.matched_report_id,
                    issue_cluster_id: analysis.issue_cluster_id,
                    state: state,
                    latitude: latVal,
                    longitude: lonVal,
...
const MatchingAPI = {
    ...
    async supportProblem(problemId, userId) {
        if (this.isBackendLive) {
            try {
                const uid = userId || (typeof getOrSetCitizenUserId === "function" ? getOrSetCitizenUserId() : "citizen_user");
    ...
    async getReport(problemId) {
        if (this.isBackendLive) {
            try {
                const res = await fetch(`${this.backendBaseUrl}/api/reports/${encodeURIComponent(problemId)}`);
                if (res.ok) return await res.json();
            } catch (e) {
                console.warn("Backend fetch report failed:", e);
            }
        }
        return null;
    },
...
/**
 * Persistent citizen user identification
 */
function getOrSetCitizenUserId() {
    try {
        let uid = localStorage.getItem("samadhan_citizen_id");
        if (!uid) {
            uid = "citizen_" + Math.random().toString(36).substring(2, 9) + "_" + Date.now().toString().slice(-4);
            localStorage.setItem("samadhan_citizen_id", uid);
        }
        return uid;
    } catch (e) {
        return "citizen_user_local";
    }
}
window.getOrSetCitizenUserId = getOrSetCitizenUserId;
...
function triggerDuplicateDetectionDebounced() {
    ...
    duplicateDetectionTimer = setTimeout(async () => {
        ...
        const district = districtSelect ? districtSelect.value : "Ranchi";
        const locality = areaInput ? areaInput.value.trim() : "";
        let repLat = latField && latField.value ? parseFloat(latField.value) : null;
        let repLon = lonField && lonField.value ? parseFloat(lonField.value) : null;
        ...
        if (!title || title.length < 5) {
            renderDuplicateWarningCard({ is_duplicate: false, duplicate_score: 0 });
            return;
        }
        ...
        let backendRes = null;
        try {
            backendRes = await MatchingAPI.checkDuplicate({
                title: title,
                description: desc,
                category: category,
                district: district,
            ...
            console.warn("Backend duplicate check request error:", e);
        }
        ...
        // 2. Local Fallback Duplicate Engine (Offline / Standalone)
        const localRes = performLocalDuplicateDetection(title, desc, category, district, locality, repLat, repLon);
        renderDuplicateWarningCard(localRes);
    }, 350);
}

/**
 * Local duplicate detection algorithm conforming to exact 50/30/10/10 formula:
 * - Text Similarity: 50%
 * - Location Match: 30%
 * - Category Match: 10%
 * - District / Locality Match: 10%
 * Status Bands:
 * 0–39: NO SIMILAR REPORT
 * 40–69: SIMILAR REPORT FOUND
 * 70–84: POTENTIAL DUPLICATE
 * 85–100: STRONG DUPLICATE MATCH
 */
function performLocalDuplicateDetection(title, description, category, district, locality, repLat, repLon) {
    if (!title || title.length < 5) {
        return { is_duplicate: false, duplicate_score: 0, status: "NO_SIMILAR_REPORT", status_label: "No Similar Report", best_match: null, matches: [] };
    }
    ...
    const normDesc = (description || "").toLowerCase().replace(/[^\w\s]/g, " ").replace(/\s+/g, " ").trim();
    const descWords = new Set(normDesc.split(" ").filter(w => w.length > 2));

    for (const prob of problems) {
        // 1. Text Similarity (50%)
        const pTitle = (prob.title || "").toLowerCase().replace(/[^\w\s]/g, " ").replace(/\s+/g, " ").trim();
        const pTitleWords = new Set(pTitle.split(" ").filter(w => w.length > 2));
        const pDesc = (prob.description || "").toLowerCase().replace(/[^\w\s]/g, " ").replace(/\s+/g, " ").trim();
        const pDescWords = new Set(pDesc.split(" ").filter(w => w.length > 2));

        let titleOverlap = 0;
        titleWords.forEach(w => { if (pTitleWords.has(w)) titleOverlap++; });
        const titleUnion = new Set([...titleWords, ...pTitleWords]).size;
        const titleSim = titleUnion > 0 ? (titleOverlap / titleUnion) : 0.0;

        let descSim = titleSim;
        if (descWords.size > 0 && pDescWords.size > 0) {
            let descOverlap = 0;
            descWords.forEach(w => { if (pDescWords.has(w)) descOverlap++; });
            const descUnion = new Set([...descWords, ...pDescWords]).size;
            descSim = descUnion > 0 ? (descOverlap / descUnion) : 0.0;
        }

        const combinedTextSim = descWords.size > 0 ? (0.60 * titleSim + 0.40 * descSim) : titleSim;
        const textScore = Math.min(100, Math.round(combinedTextSim * 100));
        ...
        if (repLat !== null && repLon !== null && probLat !== null && probLon !== null) {
            distMeters = calculateHaversineDistance(repLat, repLon, probLat, probLon);
            if (distMeters !== null) {
                distKm = parseFloat((distMeters / 1000.0).toFixed(2));
                if (distMeters <= 100) locScore = 100;
                else if (distMeters <= 500) locScore = 80;
                else if (distMeters <= 1000) locScore = 50;
                else if (distMeters <= 5000) locScore = 20;
                else locScore = 0;
            }
        } else if (prob.district && district && prob.district.toLowerCase() === district.toLowerCase()) {
            locScore = 40; // District match fallback
        }
        ...
        if (prob.category && category && prob.category.toLowerCase() === category.toLowerCase()) {
            catScore = 100;
        } else if (prob.category && category && (prob.category.toLowerCase().includes(category.toLowerCase()) || category.toLowerCase().includes(prob.category.toLowerCase()))) {
            catScore = 70;
        }

        // 4. District / Locality Match (10%)
        let areaScore = 0;
        const probLoc = (prob.locality || prob.village || prob.location || "").toLowerCase().trim();
        const inputLoc = (locality || "").toLowerCase().trim();
        if (inputLoc && probLoc && inputLoc === probLoc) {
            areaScore = 100;
        } else if (prob.district && district && prob.district.toLowerCase() === district.toLowerCase()) {
            areaScore = 70;
        }

        // Final 50/30/10/10 Score
        let rawScore = (textScore * 0.50) + (locScore * 0.30) + (catScore * 0.10) + (areaScore * 0.10);
        if (catScore === 0 && textScore < 35) {
            rawScore = Math.min(35, rawScore * 0.60);
        }
        const finalScore = Math.round(Math.min(100, Math.max(0, rawScore)));

        let status = "NO_SIMILAR_REPORT";
        let statusLabel = "No Similar Report";
        let isDup = false;
        if (finalScore >= 85) {
            status = "STRONG_DUPLICATE_MATCH";
            statusLabel = "Strong Duplicate Match";
            isDup = true;
            ...
            status = "SIMILAR_REPORT_FOUND";
            statusLabel = "Similar Report Found";
            isDup = true;
        }

        let reasons = [];
        if (textScore >= 80) reasons.push(`High text similarity (${textScore}%)`);
        else if (textScore >= 45) reasons.push(`Moderate text overlap (${textScore}%)`);
        if (distMeters !== null) {
            if (distMeters <= 100) reasons.push(`Immediate vicinity (${Math.round(distMeters)} m)`);
            else if (distMeters <= 1000) reasons.push(`Nearby location (${Math.round(distMeters)} m)`);
            else if (distKm <= 10) reasons.push(`Same area (${distKm.toFixed(1)} km)`);
        } else if (areaScore >= 70) {
            reasons.push(`Same district (${district})`);
        }
        if (catScore >= 70) reasons.push(`Category match: ${prob.category}`);

        const clusterId = prob.issue_cluster_id || (finalScore >= 70 ? `CLUSTER-${prob.id}` : null);
        ...
        evaluated.push({
            ...
            locality: prob.locality || prob.location,
            distance_meters: distMeters,
            distance_km: distKm,
            text_similarity: textScore,
            location_similarity: locScore,
            category_similarity: catScore,
            area_similarity: areaScore,
            overall_similarity: finalScore,
            duplicate_score: finalScore,
            status: status,
            status_label: statusLabel,
            is_duplicate: isDup,
            support_count: prob.support_count || 1,
    ...
    evaluated.sort((a, b) => b.duplicate_score - a.duplicate_score);

    if (evaluated.length === 0 || evaluated[0].duplicate_score < 40) {
        return {
            duplicate_score: evaluated.length > 0 ? evaluated[0].duplicate_score : 0,
            status: "NO_SIMILAR_REPORT",
            status_label: "No Similar Report",
            is_duplicate: false,
            best_match: null,
            matches: []
        };
    ...
    return {
        duplicate_score: top.duplicate_score,
        status: top.status,
        status_label: top.status_label,
        is_duplicate: true,
        best_match: top,
        matches: evaluated.slice(0, 5)
    };
}

/**
 * Render duplicate warning card into DOM conforming to section 11 & 20
 */
function renderDuplicateWarningCard(data) {
    const card = document.getElementById("aiDuplicateCard");
    if (!card) return;

    if (!data || data.duplicate_score < 40 || (!data.best_match && (!data.matches || data.matches.length === 0))) {
        card.style.display = "none";
        window.currentDuplicateResult = null;
        return;
    }

    const match = data.best_match || data.matches[0];
    const score = data.duplicate_score !== undefined ? data.duplicate_score : (match.duplicate_score || match.overall_similarity || 0);

    window.currentDuplicateResult = {
        id: match.report_id || match.id,
        report_id: match.report_id || match.id,
        title: match.title,
        description: match.description || "",
        category: match.category,
        district: match.district || match.location || "Ranchi",
        locality: match.locality || match.district || "Local Ward",
        duplicate_score: score,
        status: data.status || match.status || "POTENTIAL_DUPLICATE",
        status_label: data.status_label || match.status_label || "Potential Duplicate",
        distance_meters: match.distance_meters,
        distance_km: match.distance_km,
        support_count: match.support_count || 1,
        issue_cluster_id: match.issue_cluster_id || `CLUSTER-${match.report_id || match.id}`,
        reasons: match.matching_reasons || match.reasons || [],
        text_similarity: match.text_similarity || 0,
        location_similarity: match.location_similarity || 0,
        category_similarity: match.category_similarity || 0,
        area_similarity: match.area_similarity || 0
    };
    ...
    if (pill) {
        if (score >= 85) {
            ...
            pill.innerHTML = `<i class="ri-error-warning-line"></i> Strong Duplicate (${score}%)`;
        } else if (score >= 70) {
            pill.className = "status-pill status-pill-warning";
            pill.innerHTML = `<i class="ri-alert-fill"></i> Potential Duplicate (${score}%)`;
        } else {
            pill.className = "status-pill status-pill-neutral";
            pill.innerHTML = `<i class="ri-information-line"></i> Similar Problem (${score}%)`;
        }
    }

    const exId = document.getElementById("dupExistingId");
    if (exId) exId.textContent = `#${match.report_id || match.id}`;

    const exCat = document.getElementById("dupExistingCategory");
    if (exCat) exCat.textContent = match.category || "--";

    const exLoc = document.getElementById("dupExistingLocation");
    if (exLoc) exLoc.textContent = match.locality || match.district || "Ranchi";

    const exDist = document.getElementById("dupExistingDistance");
    if (exDist) {
        if (match.distance_meters !== undefined && match.distance_meters !== null) {
            exDist.textContent = match.distance_meters < 1000
                ? `${Math.round(match.distance_meters)} meters`
                : `${(match.distance_meters / 1000.0).toFixed(1)} km`;
        } else if (match.distance_km !== undefined && match.distance_km !== null) {
            exDist.textContent = `${match.distance_km.toFixed(1)} km`;
        } else {
            exDist.textContent = "Same District";
        }
    }

    const exStatus = document.getElementById("dupExistingStatus");
    if (exStatus) exStatus.textContent = data.status_label || match.status_label || "Potential Duplicate";

    const exSupp = document.getElementById("dupExistingSupportCount");
    if (exSupp) exSupp.textContent = match.support_count || 1;

    const exTitle = document.getElementById("dupExistingTitle");
    if (exTitle) exTitle.textContent = `“${match.title}”`;

    const chipsWrap = document.getElementById("dupReasonsChips");
    if (chipsWrap) {
        const reasons = (match.matching_reasons && match.matching_reasons.length > 0)
            ? match.matching_reasons
            : (match.reasons || ["Text similarity", "Nearby location"]);
        chipsWrap.innerHTML = reasons.map(r => `<span class="reason-chip">${escapeHtml(r)}</span>`).join("");
    }
}

/**
 * Choice 1: Citizen chooses to Support Existing Report
 */
async function supportExistingProblemAction() {
    ...
    const citizenUserId = getOrSetCitizenUserId();

    // Call live REST API with persistent citizen ID
    const apiRes = await MatchingAPI.supportProblem(dupId, citizenUserId);

    if (apiRes && apiRes.status === 409) {
        showToast("ℹ You have already supported this report.");
        return;
    }
    ...
    const newCnt = (apiRes && apiRes.data && apiRes.data.support_count) ? apiRes.data.support_count : (prob ? prob.support_count : 2);
    showToast(`✓ Thank you! Your support has been added to this issue.`);

    addNotification(
        "Admin",
        dupId,
        `Community Support Endorsed for #${dupId}`,
        `Citizen ${citizenUserId} supported existing problem #${dupId}. Total supporters: ${newCnt}.`
    );

    renderProblemsList();
    updateIntegrityDashboardStats();

    // Reset form and return to problems view
    setTimeout(() => {
...
/**
 * Choice 2: Citizen chooses Report Anyway (Opens Confirmation Dialog)
 */
function reportAnywayAction() {
    if (!window.currentDuplicateResult) return;
    const modal = document.getElementById("modalReportAnywayConfirm");
    if (modal) {
        modal.style.display = "flex";
    }
}

function closeReportAnywayModal() {
    const modal = document.getElementById("modalReportAnywayConfirm");
    if (modal) {
...
function confirmSubmitReportAnyway() {
    closeReportAnywayModal();
    window.overrideDuplicateReport = true;
    showToast("Proceeding: Submitting independent report linked to community cluster.");

    const pill = document.getElementById("duplicateStatusPill");
    if (pill) {
        pill.className = "status-pill status-pill-neutral";
        pill.innerHTML = '<i class="ri-check-line"></i> Confirmed: Report Anyway';
    }
...
/**
 * Choice 3: View Existing Report Details
 */
function viewExistingDuplicateModal() {
    if (!window.currentDuplicateResult || !window.currentDuplicateResult.id) return;
    const r = window.currentDuplicateResult;

    const modal = document.getElementById("modalViewExistingProblem");
    if (!modal) return;

    const fId = document.getElementById("modalDupReportId");
    if (fId) fId.textContent = `#${r.id}`;

    const fTitle = document.getElementById("modalDupTitle");
    if (fTitle) fTitle.textContent = r.title;

    const fDesc = document.getElementById("modalDupDesc");
    if (fDesc) fDesc.textContent = r.description || "Detailed citizen report on municipal/societal problem.";

    const fCat = document.getElementById("modalDupCategory");
    if (fCat) fCat.textContent = r.category || "--";

    const fLoc = document.getElementById("modalDupLocation");
    if (fLoc) fLoc.textContent = `${r.locality ? r.locality + ', ' : ''}${r.district || 'Jharkhand'}`;

    const fDist = document.getElementById("modalDupDistance");
    if (fDist) {
        if (r.distance_meters !== undefined && r.distance_meters !== null) {
            fDist.textContent = r.distance_meters < 1000
                ? `${Math.round(r.distance_meters)} meters`
                : `${(r.distance_meters / 1000.0).toFixed(1)} km`;
        } else {
            fDist.textContent = "Immediate local area";
        }
    }

    const fSupp = document.getElementById("modalDupSupportCount");
    if (fSupp) fSupp.textContent = `${r.support_count || 1} citizen${(r.support_count || 1) > 1 ? 's' : ''}`;

    const fCluster = document.getElementById("modalDupCluster");
    if (fCluster) fCluster.textContent = r.issue_cluster_id || `CLUSTER-${r.id}`;

    const fSignals = document.getElementById("modalDupSignals");
    if (fSignals) {
        fSignals.innerHTML = `
            <span class="reason-chip">Text Match: ${r.text_similarity || Math.round(r.duplicate_score * 0.9)}%</span>
            <span class="reason-chip">Location Match: ${r.location_similarity || 80}%</span>
            <span class="reason-chip">Category Match: ${r.category_similarity || 100}%</span>
            <span class="reason-chip">Area Match: ${r.area_similarity || 70}%</span>
        `;
    }
...
function closeViewExistingModal() {
    const modal = document.getElementById("modalViewExistingProblem");
    if (modal) {
        modal.style.display = "none";
    }
...
function supportFromViewModal() {
    closeViewExistingModal();
    supportExistingProblemAction();
}

window.getOrSetCitizenUserId = getOrSetCitizenUserId;
window.triggerDuplicateDetectionDebounced = triggerDuplicateDetectionDebounced;
window.performLocalDuplicateDetection = performLocalDuplicateDetection;
window.renderDuplicateWarningCard = renderDuplicateWarningCard;
window.supportExistingProblemAction = supportExistingProblemAction;
window.reportAnywayAction = reportAnywayAction;
window.closeReportAnywayModal = closeReportAnywayModal;
window.confirmSubmitReportAnyway = confirmSubmitReportAnyway;
window.viewExistingDuplicateModal = viewExistingDuplicateModal;
window.closeViewExistingModal = closeViewExistingModal;
window.supportFromViewModal = supportFromViewModal;
