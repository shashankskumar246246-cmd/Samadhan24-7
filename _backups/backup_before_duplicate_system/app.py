...
"""
    ...
    def do_POST(self):
        ...
        if m_supp:
            ...
            """, (pid, f"Report #{pid} received community endorsement. Total supporters: {new_cnt}."))
            conn.commit()
            conn.close()
            return self._send_json({
                "message": f"Supported existing report #{pid} successfully!",
                "problem_id": pid,
        ...
        # 3. Submit a problem: POST /api/problems
        if path == "/api/problems":
            title = body.get("title", "").strip()
            desc = body.get("description", "").strip()
            ...
            submitted_by = body.get("submitted_by", "Citizen")
            duplicate_status = body.get("duplicate_status", "Original")
            duplicate_of_id = body.get("duplicate_of_report_id")
            ...
            if not target_dup_id and dup_res.get("existing_report"):
                target_dup_id = dup_res["existing_report"]["id"]
