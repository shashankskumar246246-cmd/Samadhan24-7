...
DUPLICATE_CONFIG = {
    "WEIGHTS": {
        "text": 0.50,         # Title + Description NLP similarity (50%)
        "location": 0.30,     # Haversine geographic proximity (30%)
        "category": 0.10,     # Category domain match (10%)
        "area": 0.10          # District / Locality match (10%)
    },
    "THRESHOLDS": {
        "NO_SIMILAR_MAX": 39,       # 0 - 39%: No similar report
        "SIMILAR_MIN": 40,          # 40 - 69%: Similar report found
        "POTENTIAL_DUP_MIN": 70,    # 70 - 84%: Potential duplicate
        "STRONG_DUP_MIN": 85        # 85 - 100%: Strong duplicate match
    },
    "LOCATION_BANDS_METERS": {
        "VERY_STRONG": 100,         # 0–100 m  -> 100% location match
        "STRONG": 500,              # 100–500 m -> 80% location match
        "MODERATE": 1000,           # 500m–1km -> 50% location match
        "WEAK_MAX": 5000            # 1km–5km  -> 20% location match, >5km -> 0%
    }
}
...
def normalize_report_text(text: str) -> str:
    ...
    - punctuation cleanup
    - unnecessary duplicate words cleanup
    - basic text normalization
    Preserves original report text for display.
    """
    if not text:
        return ""
    cleaned = text.lower()
...
def calculate_text_similarity(new_title: str, new_desc: str, ext_title: str, ext_desc: str) -> float:
    """
    Calculates normalized text similarity between 0 and 100.
    Combines title and description with TF-IDF cosine similarity + token overlap.
    """
    norm_new_t = normalize_report_text(new_title)
    norm_ext_t = normalize_report_text(ext_title)
    norm_new_d = normalize_report_text(new_desc)
    norm_ext_d = normalize_report_text(ext_desc)

    # 1. Title Similarity
    toks_new_t = tokenize(norm_new_t)
    toks_ext_t = tokenize(norm_ext_t)
    if toks_new_t and toks_ext_t:
        tf_new_t = compute_term_frequencies(toks_new_t)
        tf_ext_t = compute_term_frequencies(toks_ext_t)
        title_cos = cosine_similarity(tf_new_t, tf_ext_t)
        title_jac = calculate_jaccard(toks_new_t, toks_ext_t)
        title_sim = 0.65 * title_cos + 0.35 * title_jac
    else:
        title_sim = 0.0

    # 2. Description Similarity
    toks_new_d = tokenize(norm_new_d)
    toks_ext_d = tokenize(norm_ext_d)
    if toks_new_d and toks_ext_d:
        tf_new_d = compute_term_frequencies(toks_new_d)
        tf_ext_d = compute_term_frequencies(toks_ext_d)
        desc_cos = cosine_similarity(tf_new_d, tf_ext_d)
        desc_jac = calculate_jaccard(toks_new_d, toks_ext_d)
        desc_sim = 0.70 * desc_cos + 0.30 * desc_jac
    else:
        desc_sim = title_sim

    # If description is present, weigh title 60% and description 40%
    if toks_new_d and toks_ext_d:
        combined = 0.60 * title_sim + 0.40 * desc_sim
    else:
        combined = title_sim

    return round(min(100.0, max(0.0, combined * 100.0)), 1)


def calculate_location_similarity(dist_meters: Optional[float], is_same_district: bool = False) -> Tuple[float, str]:
    """
    Calculates location similarity score between 0 and 100 using Haversine distance.
    Interpretation:
    0–100 m       = Very Strong Location Match (100)
    100–500 m     = Strong Location Match (80)
    500 m–1 km    = Moderate Location Match (50)
    1 km–5 km     = Weak Location Match (20)
    >5 km         = Distant / No Location Match (0)
    """
    if dist_meters is None:
        if is_same_district:
            return 40.0, "Same District (Coordinates Missing)"
        return 0.0, "Location Unavailable"

    if dist_meters <= 100:
        return 100.0, f"Immediate vicinity ({int(dist_meters)} m)"
    elif dist_meters <= 500:
        return 80.0, f"Nearby location ({int(dist_meters)} m)"
    elif dist_meters <= 1000:
        return 50.0, f"Same neighborhood ({int(dist_meters)} m)"
    elif dist_meters <= 5000:
        return 20.0, f"Same general area ({dist_meters / 1000.0:.1f} km)"
    else:
        return 0.0, f"Distant location ({dist_meters / 1000.0:.1f} km)"
...
def calculate_duplicate_score(new_report: Dict[str, Any], existing_report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    - Location Match: 30%
    - Category Match: 10%
    - District / Locality: 10%

    Formula:
    duplicate_score = (text_similarity * 0.50) + (location_similarity * 0.30) + (category_similarity * 0.10) + (area_similarity * 0.10)
    Normalized to 0–100.
    """
    matching_reasons: List[str] = []

    # 1. Text Similarity (0–100)
    new_title = new_report.get("title", "")
    new_desc = new_report.get("description", "")
    ext_title = existing_report.get("title", "")
    ext_desc = existing_report.get("description", "")
    text_sim = calculate_text_similarity(new_title, new_desc, ext_title, ext_desc)

    if text_sim >= 80:
        matching_reasons.append(f"High text similarity ({int(text_sim)}%)")
    elif text_sim >= 50:
        matching_reasons.append(f"Moderate text overlap ({int(text_sim)}%)")

    # 2. Location Similarity (0–100) via Haversine
    ...
    if new_lat is not None and new_lon is not None and ext_lat is not None and ext_lon is not None:
        try:
            dist_km = calculate_haversine_distance_km(float(new_lat), float(new_lon), float(ext_lat), float(ext_lon))
    ...
    is_same_district = bool(dist_new and dist_ext and dist_new == dist_ext)

    loc_sim, loc_reason = calculate_location_similarity(dist_meters, is_same_district)
    if loc_sim >= 50:
        matching_reasons.append(loc_reason)
    ...
    if cat_new and cat_ext and cat_new.lower() == cat_ext.lower():
        ...
        cat_sim = 70.0
        matching_reasons.append(f"Related category: {cat_new}")
    else:
        cat_sim = 0.0

    # 4. District / Locality Match (0–100)
    loc_new = (new_report.get("locality") or new_report.get("village") or new_report.get("area") or "").strip().lower()
    loc_ext = (existing_report.get("locality") or existing_report.get("village") or existing_report.get("area") or "").strip().lower()

    if loc_new and loc_ext and loc_new == loc_ext:
        area_sim = 100.0
        matching_reasons.append(f"Identical locality: {new_report.get('locality') or loc_new}")
    ...
    # DIFFERENT PROBLEM PROTECTION (TEST 4):
    # If category completely differs and text similarity is low, it is an unrelated problem occurring
    # at the same location, NOT a duplicate report.
    if cat_sim == 0.0 and text_sim < 35.0:
        raw_score = min(35.0, raw_score * 0.60)

    final_score = int(round(min(100.0, max(0.0, raw_score))))

    # Determine Duplicate Status
    if final_score >= DUPLICATE_CONFIG["THRESHOLDS"]["STRONG_DUP_MIN"]:
        status = "STRONG_DUPLICATE_MATCH"
        status_label = "Strong Duplicate Match"
        is_duplicate = True
    elif final_score >= DUPLICATE_CONFIG["THRESHOLDS"]["POTENTIAL_DUP_MIN"]:
        status = "POTENTIAL_DUPLICATE"
        status_label = "Potential Duplicate"
        is_duplicate = True
    elif final_score >= DUPLICATE_CONFIG["THRESHOLDS"]["SIMILAR_MIN"]:
        status = "SIMILAR_REPORT_FOUND"
        status_label = "Similar Report Found"
        is_duplicate = True
    else:
    ...
    # Issue Clustering
    issue_cluster_id = existing_report.get("issue_cluster_id")
    if not issue_cluster_id and final_score >= DUPLICATE_CONFIG["THRESHOLDS"]["POTENTIAL_DUP_MIN"]:
        issue_cluster_id = f"CLUSTER-{existing_report.get('id', '1001')}"
    ...
    return {
        ...
        "locality": existing_report.get("locality") or existing_report.get("location"),
        "distance_meters": dist_meters,
        "distance_km": dist_km,
        "text_similarity": int(round(text_sim)),
        "location_similarity": int(round(loc_sim)),
        "category_similarity": int(round(cat_sim)),
        "area_similarity": int(round(area_sim)),
        "overall_similarity": final_score,
        "duplicate_score": final_score,
        "status": status,
        "status_label": status_label,
        "is_duplicate": is_duplicate,
        "existing_status": existing_report.get("status", "Pending"),
        "created_at": existing_report.get("created_at"),
        "support_count": existing_report.get("support_count", 1),
...
def detect_duplicate_reports(new_report: Dict[str, Any], existing_reports: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Evaluates new draft report against relevant existing reports to detect duplicate or similar submissions.
    Features:
    - Pre-filters candidate pool for performance scalability
    - Calculates 50/30/10/10 multi-signal duplicate similarity
    - Returns top 3–5 candidate matches sorted by highest duplicate score
    - Preserves issue clusters and original reports
    """
    if not existing_reports or not (new_report.get("title") or "").strip():
        return {
            "duplicate_score": 0,
            "status": "NO_SIMILAR_REPORT",
            "status_label": "No Similar Report",
    ...
    new_district = (new_report.get("district") or "").strip().lower()
    new_cat = normalize_category_name(new_report.get("category", "")).lower()
    new_lat = new_report.get("latitude")
    new_lon = new_report.get("longitude")

    # Scalable Candidate Pre-filtering
    # If total reports > 50, filter candidate pool by district, category, or bounding box
    if len(existing_reports) > 50:
        candidates = []
        for ext in existing_reports:
            if new_id and ext.get("id") == new_id:
                continue
            ext_district = (ext.get("district") or "").strip().lower()
            ext_cat = normalize_category_name(ext.get("category", "")).lower()
            ext_lat = ext.get("latitude")
            ext_lon = ext.get("longitude")

            # Same district
            if new_district and ext_district and new_district == ext_district:
                candidates.append(ext)
                continue
            # Same category
            if new_cat and ext_cat and (new_cat == ext_cat or new_cat in ext_cat or ext_cat in new_cat):
                candidates.append(ext)
                continue
            # Bounding box (~0.5 deg approx 55 km)
            if new_lat is not None and new_lon is not None and ext_lat is not None and ext_lon is not None:
                try:
                    if abs(float(new_lat) - float(ext_lat)) <= 0.5 and abs(float(new_lon) - float(ext_lon)) <= 0.5:
                        candidates.append(ext)
                        continue
                except (ValueError, TypeError):
                    pass
    else:
        candidates = [ext for ext in existing_reports if not (new_id and ext.get("id") == new_id)]
    ...
    for ext in candidates:
        match_res = calculate_duplicate_score(new_report, ext)
        evaluated.append(match_res)

    # Sort candidates by highest overall similarity / duplicate score
    ...
    is_duplicate = top["is_duplicate"]

    # Top 3-5 strongest matches
    top_matches = evaluated[:5]
    ...
    return {
        ...
        "status_label": status_label,
        "is_duplicate": is_duplicate,
        "best_match": top if is_duplicate else None,
        "matches": top_matches if is_duplicate else []
    }
